...
const pkg = require('../package.json');
 // use package name and version rom package.json
...
exports.register.attributes = { // hapi requires attributes for a plugin.
  pkg                      // also see: http://hapijs.com/tutorials/plugins
};
...
internals.implementation = function (server, options) {
  ...
  return {
    ...
    authenticate(request, reply) {
      ...
      if (options.key && typeof options.validateFunc === 'function') {
        ...
        keyFunc(decoded, (err, key, extraInfo) => {
          ...
          const keys = (internals.isArray(key)) ? key : [key];
          let keysTried = 0;
              ...
              (verify_err, verify_decoded) => {
                if (verify_err) {
                  ...
                  return options.validateFunc(verify_decoded, request,
                    (validate_err, valid, credentials) => { // bring your own checks
                      if (validate_err) {
        ...
        return options.verifyFunc(decoded, request,
          (verify_error, valid, credentials) => {
            if (verify_error) {
    ...
    payload: (request, reply) => {
      if (_.isFunction(options.validatePayloadFunc) === true) {
        return options.validatePayloadFunc(request, reply);
      }
